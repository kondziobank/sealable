...
export default new (class SignInForm extends Form<typeof fields, void> {
	...
	async validateValues(
		...
		if (user.empty) {
			...
			return { valid: false, error: `Something really wrong happened` };
                }

                if (!user.items[0].get('confirmed')) {
			return { valid: false, error: `User is not confirmed.` };
                }
	...
	async onSubmit(ctx: Context, data: FormData) {
                if (data.raw_values.password as string == "") {
			throw new Error("User tried ");
                }
		try {
